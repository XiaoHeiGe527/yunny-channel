...
<div class="upload-container">
    <h2 class="layui-h2" style="text-align: center; margin-bottom: 30px;">技术文件上传</h2>
    ...
    layui.use(['layer', 'form', 'upload', 'element', 'jquery', 'laydate'], function() {
        ...
        function initArchiveTypeSelect() {
            $.ajax({
                ...
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify({category: "档案类型", remarks: "技术文件档案"}),
                success: function(response) {
                    if (response && response.code === 200 && response.data) {
        ...
        function initDatePickers() {
            ...
            laydate.render({
                elem: '#signatureTime',
                type: 'datetime', // 修改为datetime类型
                format: 'yyyy-MM-dd HH:mm:ss', // 修改为完整日期时间格式
