    ...
    layui.use(['laypage', 'layer'], function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.code === 200) {
                        ...
                        // 根据权限设置标志
                        hasAddPermission = permissions.includes('/renShiViewJump/chemicalFile/handleFileUpload');
        ...
        function loadFileTypes() {
            $.ajax({
                ...
                success: function (response) {
                    if (response.code === 200) {
                        var fileTypes = response.data;
                        var fileTypeSelect = document.getElementById('fileType');
                        fileTypeSelect.innerHTML = '<option value="">请选择档案类型</option>';
                        ...
                        alert('获取档案类型数据失败：' + response.message);
                    }
                },
                error: function () {
                    alert('请求发生错误，请检查网络或接口地址。');
        ...
        function renderTable(data) {
            ...
            for (var m = 0; m < editButtons.length; m++) {
                editButtons[m].onclick = function() {
                    ...
                    form.action = '/renShiViewJump/chemicalFile/chemicalFileEdit';
                    form.method = 'post';
        ...
        document.getElementById('addButton').onclick = function () {
            var form = document.createElement('form');
            form.action = '/renShiViewJump/chemicalFile/handleFileUpload';
            form.method = 'post';
